...
SOURCE_RELIABILITY = {
    ...
    "gdp_g":     0.9,  # Eurostat/FMI
    "tourists":  0.8,  # UNWTO/Eurostat
    "connect":   0.5,  # estimation proxy
    "pol_risk":  0.6,  # Coface/subjectif
    "reg_stab":  0.5,  # subjectif 1-5
    "geo_exp":   0.5,  # subjectif 1-5
    # HVS 2025 European Hotel Valuation Index
    "hvi_index": 0.85,  # HVS — données réelles marché
    "hvi_cagr":  0.80,  # HVS — CAGR 10 ans fiable
    # CBRE Destination Index 2025
    "labour_cost": 0.80,  # CBRE/Eurostat
    "hwe_score":   0.70,  # CBRE/Oxford Economics
    "cert_pct":  0.5,  # estimation partielle
    "reg_esg":   0.5,  # subjectif 1-5
    "tax_eu":    0.6,  # EU Taxonomy proxy
    # Faisabilité développement
    "dev_cost":      0.65,  # estimation marché
    "dev_revpar_idx":0.70,  # calculé depuis RevPAR + dev_cost
    "yield_on_cost": 0.65,  # calculé
}

# Paramètres de transformation non-linéaire par variable
# type: "linear" | "log" | "sqrt" | "sigmoid" | "diminishing"
# Les variables à rendements décroissants (ex: occ >80% = peu de upside)
NONLINEAR = {
    "occ":             "diminishing",
    "revpar":          "sqrt",
    "adr":             "sqrt",
    ...
    "geo_exp":         "linear",
    # O'Neill 2023
    "goppar_rsd":      "linear",   # déjà un % — direction=-1
    ...
    "tax_eu":          "linear",
    # HVS 2025
    "hvi_index":       "sqrt",     # valeurs 0.3–4.3 — sqrt atténue l'écart Paris/Tunis
    "hvi_cagr":        "linear",
    # CBRE 2025
    "labour_cost":     "linear",
    "hwe_score":       "linear",
    # Faisabilité
    "dev_cost":        "log",      # coûts : outliers importants (Londres vs Tunis)
    "dev_revpar_idx":  "sqrt",
