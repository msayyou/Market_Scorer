...
def _apply_shocks(market: dict, shocks: dict) -> dict:
    """
...
def stress_test_markets(
    ...
    # Scores par scénario — calculés sur le panel entier à chaque fois
    scenario_scores = {}
    for scen_id, scen in STRESS_SCENARIOS.items():
        shocked_markets = [_apply_shocks(m, scen["shocks"]) for m in markets]
